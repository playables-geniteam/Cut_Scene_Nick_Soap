...
export class CharacterMovement extends Component {
    ...
    // Set false to freeze the character in place (stops movement + zeros velocity)
    ...
    start() {
        ...
        myCollider?.on('onCollisionEnter', (target: ICollisionEvent) => {
            if (target.otherCollider != target.selfCollider && this._isInTheAir) {
                this.onLand();
            }
        });
    }
    ...
    update(deltaTime: number) {
        if (this._isMoving) {
            // If movement was disabled mid-move, stop immediately
            if (!this.canMove) {
                this.onMovementRelease();
                return;
            }
    ...
    onMovement(degree: number, offset: number) {
        if (!this.canMove) return; // block input when frozen
    ...
    onJump(btnName: string) {
        if (!this.canMove) return; // block jump when frozen
    ...
    /** Freezes the character in place and plays idle. */
    public freeze(): void {
        this.canMove = false;
        this.onMovementRelease();
    }
